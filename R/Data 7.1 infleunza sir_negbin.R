...
infected_quantiles <- quantiles_s_i[15:28]
...
if2_log_epidemic <- function(
  ...
  log_start <- log(start_params)

  p <- length(log_start)
  # Initialize parameter swarm: each row is a log-parameter vector
  log_params <- matrix(log_start, nrow = Np, ncol = p, byrow = TRUE)
  colnames(log_params) <- names(start_params)

  for (m in seq_len(Nmif)) {
    # Every 10th
    if (m %% 10 == 0) {
      cat("IF2 iteration", m, "of", Nmif, "\n")
    }
    # SD for random walk at this iteration
    sd_m <- rw_sd * cooling_fraction^(m - 1)

    # --- Initialize filtered particles at time 0 ---
    log_Theta_F <- log_params + matrix(
      rnorm(Np * p, sd = sd_m), nrow = Np, ncol = p
    )
    colnames(log_Theta_F) <- names(start_params)
    X_F <- init_fn_epidemic(Np)

    # --- Loop through observations ---
    for (n in seq_along(y)) {
      log_Theta_P <- log_Theta_F + matrix(
        rnorm(Np * p, sd = sd_m), nrow = Np, ncol = p
      )
      colnames(log_Theta_P) <- names(start_params)
      Theta_P <- exp(log_Theta_P)

      # propagate all state particles
      ...
      # resample filtered particles
      log_Theta_F <- log_Theta_P[idx, , drop = FALSE]
      X_F         <- X_P[idx, , drop = FALSE]
    }

    # recycle filtered swarm for next iteration
    log_params <- log_Theta_F
  }

  # final estimate: exponentiated mean of log-parameters
  theta_est <- exp(colMeans(log_params))
