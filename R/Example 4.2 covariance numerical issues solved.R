library(ggplot2)
library(tibble)
library(extraDistr) # for rht and dht: half-t functions
...
# Main function repeating the above many times

main_function <- function(k, prop_sd = c(0.1, 0.1, 0.1), pilot_M = 2000,
                          pilot_N = 100, pilot_reps = 10, M_iter = 15000,
                          burn_in = 2000) {
  for (i in 1:k) {
    # Simulate data
    T_val <- 50
    ...
    print(paste("Mean phi estimate:", phi_estimates))
    print(paste("Mean sigma_x estimate:", sigma_x_estimates))
    print(paste("Mean sigma_y estimate:", sigma_y_estimates))
  }
}
